...
void main()
{
    ...
    if (oSpeaker != gsLIGetTarget())           return;
    ...
    //filter feedback
    string sOutput       = GetLocalString(OBJECT_SELF, "GS_LI_OUTPUT");
    DeleteLocalString(OBJECT_SELF, "GS_LI_OUTPUT");
    ...
    if (nSpeakLanguage || nLanguage1)
    {
        ...
        if (gsLAGetCanSpeakLanguage(nLanguage1, oSpeaker))
        {
            ...
            if (sString != "")
            {
                ...
                //save output for feedback filter
                SetLocalString(OBJECT_SELF, "GS_LI_OUTPUT", sOutput);
