...
void gsRun(object oTarget, int nSpell, int nMetaMagic, int nDuration)
{
    if (! nDuration)        return;
    ...
    effect eEffect =
        EffectLinkEffects(
            ...
            EffectDamage(
                gsSPGetDamage(1, 6, nMetaMagic),
                DAMAGE_TYPE_ACID));

    gsSPApplyEffect(oTarget, eEffect, nSpell);

    DelayCommand(6.0, gsRun(oTarget, nSpell, nMetaMagic, nDuration - 1));
}
//----------------------------------------------------------------
void main()
{
    ...
    if (! GetHasSpellEffect(nSpell, oTarget))
    {
        ...
        DelayCommand(
            fDelay + 6.0,
            gsRun(oTarget, nSpell, nMetaMagic, nDuration));
    }
}
