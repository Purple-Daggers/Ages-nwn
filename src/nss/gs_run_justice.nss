...
int gsDetermineTrial()
{
    ...
    while (nTrial)
    {
        ...
        if (nTimestamp > stTrial.nTimestamp)
        {
            ...
            if (nJudgement < 40)
            {
                nDuration                 = gsTIGetGameTimestamp(stTrial.nPenalty) / 40 * (40 - nJudgement);
            ...
            {
                nDuration                 = gsTIGetGameTimestamp(stTrial.nPenalty) / 20 * (nJudgement - 80);
                if (nDuration < 3600) nDuration = 3600;
...
void main()
{
    if (gsDetermineTrial() || gsDetermineJudgement())
    {
