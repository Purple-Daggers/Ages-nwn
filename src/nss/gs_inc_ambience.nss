...
void gsAMApplyAmbience(int nType, object oArea = OBJECT_SELF, int nOption = GS_AM_OPTION_ALL)
{
    ...
    if (nMemorize || nFlag)
    {
        ...
        float fX                   = 0.0;
        float fY                   = 0.0;
        int nCountMC1              = GetLocalInt(oModule, sTagMC1 + "COUNT");
        int nCountMC2              = GetLocalInt(oModule, sTagMC2 + "COUNT");
        int nMemorizedMainColor1   = 0;
        int nMemorizedMainColor2   = 0;
        int nMemorizedSourceColor1 = 0;
        int nMemorizedSourceColor2 = 0;
        int nSourceColor1          = GetLocalInt(oModule, "GS_AM_SC1_" + sType);
        int nSourceColor2          = GetLocalInt(oModule, "GS_AM_SC2_" + sType);
        int nColor1                = 0;
        int nColor2                = 0;
        ...
        if (nMemorize)
        {
            while (fX < fSizeX)
            {
                ...
                while (fY < fSizeY)
                {
                    ...
                    SetLocalInt(oArea, "GS_AM_ML1_" + sString, nMemorizedMainColor1);
                    SetLocalInt(oArea, "GS_AM_ML2_" + sString, nMemorizedMainColor2);
                    SetLocalInt(oArea, "GS_AM_SL1_" + sString, nMemorizedSourceColor1);
                    SetLocalInt(oArea, "GS_AM_SL2_" + sString, nMemorizedSourceColor2);

                    //tile main light color
                    nColor1 = (nMemorizedMainColor1 == TILE_MAIN_LIGHT_COLOR_BLACK ||
                               nMemorizedMainColor1 == 255) ?
                               GetLocalInt(oModule, sTagMC1 + IntToString(Random(nCountMC1))) :
                               nMemorizedMainColor1;
                    nColor2 = (nMemorizedMainColor2 == TILE_MAIN_LIGHT_COLOR_BLACK ||
                               nMemorizedMainColor2 == 255) ?
                               GetLocalInt(oModule, sTagMC2 + IntToString(Random(nCountMC2))) :
        ...
        else if (nType == GS_AM_TYPE_NONE)
        {
            while (fX < fSizeX)
            {
                ...
                while (fY < fSizeY)
                {
                    ...
                    //tile main light color
                    nColor1   = GetLocalInt(oArea, "GS_AM_ML1_" + sString);
                    nColor2   = GetLocalInt(oArea, "GS_AM_ML2_" + sString);

                    SetTileMainLightColor(lLocation, nColor1, nColor2);

                    //tile source light color
                    nColor1   = GetLocalInt(oArea, "GS_AM_SL1_" + sString);
                    nColor2   = GetLocalInt(oArea, "GS_AM_SL2_" + sString);
        ...
        else
        {
            while (fX < fSizeX)
            {
                ...
                while (fY < fSizeY)
                {
                    ...
                    nMemorizedMainColor1   = GetLocalInt(oArea, "GS_AM_ML1_" + sString);
                    nMemorizedMainColor2   = GetLocalInt(oArea, "GS_AM_ML2_" + sString);
                    nMemorizedSourceColor1 = GetLocalInt(oArea, "GS_AM_SL1_" + sString);
                    nMemorizedSourceColor2 = GetLocalInt(oArea, "GS_AM_SL2_" + sString);
...
//................................................................
    ...
    if (nOption & GS_AM_OPTION_FOG)
    {
        ...
        {
            nFogColorSun   = GetLocalInt(oModule, "GS_AM_FCS_" + sType);
            nFogColorMoon  = GetLocalInt(oModule, "GS_AM_FCM_" + sType);
            nFogAmountSun  = GetLocalInt(oModule, "GS_AM_FAS_" + sType);
            nFogAmountMoon = GetLocalInt(oModule, "GS_AM_FAM_" + sType);
        }
...
//................................................................
    ...
    //sky
    if (nOption & GS_AM_OPTION_SKY)
    {
        nSkyBox = nType == GS_AM_TYPE_NONE ?
                  GetLocalInt(oArea, "GS_AM_SKY") :
                  GetLocalInt(oModule, "GS_AM_S" + sSunMoon + "_" + sType);
...
{
    sTag       += "_";
    int nCount  = GetLocalInt(OBJECT_SELF, sTag + "COUNT");
    int nNth    = nCount;
    nChance    += nCount;

    for (; nNth < nChance; nNth++)
    {
        SetLocalInt(OBJECT_SELF, sTag + IntToString(nNth), nColor);
    }

    SetLocalInt(OBJECT_SELF, sTag + "COUNT", nNth);
}
//----------------------------------------------------------------
...
{
    string sType = IntToString(nType);
    SetLocalInt(OBJECT_SELF, "GS_AM_SC1_" + sType, nColor1);
    SetLocalInt(OBJECT_SELF, "GS_AM_SC2_" + sType, nColor2);
}
//----------------------------------------------------------------
void gsAMFC(int nType, int nColorSun, int nAmountSun, int nColorMoon, int nAmountMoon)
{
    string sType = IntToString(nType);
    SetLocalInt(OBJECT_SELF, "GS_AM_FCS_" + sType, nColorSun);
    SetLocalInt(OBJECT_SELF, "GS_AM_FCM_" + sType, nColorMoon);
    SetLocalInt(OBJECT_SELF, "GS_AM_FAS_" + sType, nAmountSun);
    SetLocalInt(OBJECT_SELF, "GS_AM_FAM_" + sType, nAmountMoon);
}
//----------------------------------------------------------------
void gsAMS(int nType, int nSkySun, int nSkyMoon)
{
    string sType = IntToString(nType);
    SetLocalInt(OBJECT_SELF, "GS_AM_SS_" + sType, nSkySun);
    SetLocalInt(OBJECT_SELF, "GS_AM_SM_" + sType, nSkyMoon);
}
//----------------------------------------------------------------
void gsAMW(int nType, int nChanceRain, int nChanceSnow)
{
    string sType = IntToString(nType);
    SetLocalInt(OBJECT_SELF, "GS_AM_WR_" + sType, nChanceRain);
    SetLocalInt(OBJECT_SELF, "GS_AM_WS_" + sType, nChanceSnow);
}
//----------------------------------------------------------------
void gsAMInitialize()
{
